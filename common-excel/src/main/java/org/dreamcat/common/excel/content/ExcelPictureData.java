package org.dreamcat.common.excel.content;

import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;
import org.apache.poi.ss.usermodel.PictureData;
import org.apache.poi.ss.usermodel.Workbook;

/**
 * Create by tuke on 2021/2/14
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
public class ExcelPictureData {

    private byte[] data;
    private int pictureType;

    public static ExcelPictureData from(PictureData picture) {
        return new ExcelPictureData(picture.getData(), picture.getPictureType());
    }

    public static ExcelPictureData fromJpeg(byte[] data) {
        return new ExcelPictureData(data, Workbook.PICTURE_TYPE_JPEG);
    }

    public static ExcelPictureData fromPng(byte[] data) {
        return new ExcelPictureData(data, Workbook.PICTURE_TYPE_PNG);
    }

    public <W extends Workbook> void fill(W workbook) {
        workbook.addPicture(data, pictureType);
    }
}
